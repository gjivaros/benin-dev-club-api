...
export function rejectOnTimeout<T = void>(
	...
	setTimeout(() => {
		reject(new Error(`Timeout, the maximum duration '${maxDurationMs}ms' is exceeded.`));
	}, maxDurationMs);
	promise.then(
...
export function enqueueAsync<T extends AsyncTask>(task: T, options?: EnqueueAsyncOptions): T;
export function enqueueAsync(options?: EnqueueAsyncOptions): <T extends AsyncTask>(task: T) => T;
export function enqueueAsync(arg1?: any, arg2?: any): any {
	const singleTask: AsyncTask | undefined = typeof arg1 === "function" ? arg1 : undefined;
	...
	function toEnqueuedTask(task: AsyncTask) {
		return async (...args: any[]) => {
			...
			const stillWaitMs = nextCallTime === undefined ? 0 : nextCallTime - Date.now();
